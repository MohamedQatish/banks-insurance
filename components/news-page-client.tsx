...
export default function NewsPageClient({ allArticles, categories }: NewsPageClientProps) {
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-8">
        {/* Category tabs */}
        <div className="flex items-center gap-2 flex-wrap flex-1" role="tablist" aria-label="فلترة بالقسم">
          <button
            role="tab"
            ...
            style={{
              ...
              border: activeCategory === 'الكل' ? '1px solid #c9a227' : '1px solid #cfd8e6',
              boxShadow: activeCategory === 'الكل' ? '0 2px 8px rgba(201,162,39,0.3)' : 'none',
            }}
          >
            الكل
          </button>
          {categories.map((cat) => (
            <button
              ...
              style={{
                ...
                border: activeCategory === cat.name ? `1px solid ${cat.color}` : '1px solid #cfd8e6',
                boxShadow: activeCategory === cat.name ? `0 2px 8px ${cat.color}44` : 'none',
              }}
            >
              {cat.name}
            </button>
        ...
        <div className="relative sm:w-56" style={{ direction: 'rtl' }}>
          <Search size={15} className="absolute top-1/2 -translate-y-1/2 right-3 pointer-events-none" style={{ color: '#8a9bb8' }} aria-hidden="true" />
          <input
            type="search"
            ...
            style={{ background: '#ffffff', border: '1px solid #cfd8e6', color: '#0a1628' }}
          ...
          {featured && (
            ...
            >
              ...
              <div className="p-6 flex flex-col justify-center">
                <span className="flex items-center gap-1.5 text-xs mb-3" style={{ color: '#8a9bb8' }}>
                  <Clock size={12} aria-hidden="true" />
                  <time dateTime={featured.date}>
                    {new Date(featured.date).toLocaleDateString('ar-SY', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </time>
                </span>
                <h2 className="text-xl md:text-2xl font-bold leading-relaxed mb-3 text-balance" style={{ color: '#0a1628' }}>
                  {featured.title}
                </h2>
                ...
                <div className="p-4 flex flex-col flex-1">
                  <span className="flex items-center gap-1 text-xs mb-2" style={{ color: '#8a9bb8' }}>
                    ...
                    <time dateTime={article.date}>
                      {new Date(article.date).toLocaleDateString('ar-SY', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </time>
                  </span>
                  <h3 className="font-bold text-sm leading-relaxed line-clamp-2 flex-1 text-pretty" style={{ color: '#0a1628' }}>
                    {article.title}
                  </h3>
                  <p className="text-xs leading-relaxed line-clamp-2 mt-2" style={{ color: '#5a6a85' }}>
                    {article.excerpt}
                  </p>
                  <Link
